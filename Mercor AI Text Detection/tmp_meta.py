...
base_models=[
    ('sgd_char36', Pipeline([
        ...
        ('clf', SGDClassifier(loss='log_loss', penalty='elasticnet', l1_ratio=0.15, alpha=1e-4, max_iter=4000, class_weight='balanced', n_iter_no_change=15))
    ]), 'answer'),
    ('sgd_charwb', Pipeline([
        ('tfidf', TfidfVectorizer(analyzer='char_wb', ngram_range=(3,5), min_df=2, max_df=0.95)),
        ('clf', SGDClassifier(loss='log_loss', penalty='elasticnet', l1_ratio=0.2, alpha=5e-5, max_iter=4000, class_weight='balanced', n_iter_no_change=15))
    ]), 'answer'),
    ...
    ('logreg_char', Pipeline([
        ...
        ('clf', LogisticRegression(max_iter=4000, C=1.5, class_weight='balanced'))
    ]), 'answer'),
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(train, y)):
    X_tr_answer=train.iloc[tr_idx]['answer']
    X_val_answer=train.iloc[val_idx]['answer']
    X_tr_both=train.iloc[tr_idx][['answer','topic']]
    X_val_both=train.iloc[val_idx][['answer','topic']]
    y_tr=y[tr_idx]
    for m_idx, (name, model, kind) in enumerate(base_models):
        mdl=clone(model)
        if kind=='answer':
            mdl.fit(X_tr_answer, y_tr)
            oof_preds[val_idx, m_idx]=mdl.predict_proba(X_val_answer)[:,1]
            test_preds[:, m_idx]+=mdl.predict_proba(test['answer'])[:,1]/skf.n_splits
        else:
            mdl.fit(X_tr_both, y_tr)
            oof_preds[val_idx, m_idx]=mdl.predict_proba(X_val_both)[:,1]
...
meta_auc=roc_auc_score(y, meta.predict_proba(oof_preds)[:,1])
print('meta AUC', meta_auc)

final_test=meta.predict_proba(test_preds)[:,1]

print('Pred stats', final_test.min(), final_test.max())
