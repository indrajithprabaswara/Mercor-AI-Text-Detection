...
base_models=[
    ('sgd_char36', Pipeline([
        ...
        ('clf', SGDClassifier(loss='log_loss', penalty='elasticnet', l1_ratio=0.15, alpha=1e-4, max_iter=4000, class_weight='balanced', n_iter_no_change=20, random_state=42))
    ]), 'answer'),
    ('sgd_charwb', Pipeline([
        ('tfidf', TfidfVectorizer(analyzer='char_wb', ngram_range=(3,5), min_df=2, max_df=0.95)),
        ('clf', SGDClassifier(loss='log_loss', penalty='elasticnet', l1_ratio=0.2, alpha=5e-5, max_iter=4000, class_weight='balanced', n_iter_no_change=20, random_state=42))
    ]), 'answer'),
    ('sgd_word', Pipeline([
        ('tfidf', TfidfVectorizer(analyzer='word', ngram_range=(1,3), min_df=2, max_df=0.95, sublinear_tf=True)),
        ('clf', SGDClassifier(loss='log_loss', penalty='elasticnet', l1_ratio=0.15, alpha=5e-4, max_iter=3000, class_weight='balanced', n_iter_no_change=20, random_state=42))
    ]), 'answer'),
    ('logreg_word', Pipeline([
        ('tfidf', TfidfVectorizer(analyzer='word', ngram_range=(1,2), min_df=2, max_df=0.9)),
        ('clf', LogisticRegression(max_iter=3000, C=2.0, class_weight='balanced', solver='lbfgs', random_state=42))
    ]), 'answer'),
    ('logreg_char', Pipeline([
        ('tfidf', TfidfVectorizer(analyzer='char', ngram_range=(4,7), min_df=2, max_df=0.95)),
        ('clf', LogisticRegression(max_iter=4000, C=1.5, class_weight='balanced', solver='lbfgs', random_state=42))
    ]), 'answer'),
    ...
    ('style_logreg', Pipeline([
        ...
        ('clf', LogisticRegression(max_iter=500, C=2.0, class_weight='balanced', random_state=42))
    ]), 'both'),
]
...
full_test_preds=np.zeros((len(test), len(base_models)))
for m_idx, (name, model, kind) in enumerate(base_models):
    mdl=clone(model)
    if kind=='answer':
        mdl.fit(train['answer'], y)
        full_test_preds[:, m_idx]=mdl.predict_proba(test['answer'])[:,1]
    elif kind=='topic':
        mdl.fit(train['topic'], y)
        full_test_preds[:, m_idx]=mdl.predict_proba(test['topic'])[:,1]
    else:
        mdl.fit(train[['answer','topic']], y)
        full_test_preds[:, m_idx]=mdl.predict_proba(test[['answer','topic']])[:,1]

final_preds=meta.predict_proba(full_test_preds)[:,1]
